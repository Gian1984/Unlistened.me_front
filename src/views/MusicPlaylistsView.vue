...
import { musicPlaylistsSeo } from '@/seo/registry/index.js'
  ...
  <div class="bg-gray-950 min-h-screen">
    <div class="p-6 sm:p-8">
      ...
      <div v-if="library.playlistsLoading && !library.playlists.length" class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        <div v-for="n in 6" :key="n" class="h-24 rounded-xl border border-gray-800 bg-gray-900/40 animate-shimmer" />
      </div>
      ...
      <ul v-else class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        ...
        >
          ...
          <div class="min-w-0 flex-1">
            <p class="truncate text-sm font-semibold text-white group-hover:text-indigo-300 transition-colors">
              {{ p.name }}
            </p>
